namespace Assets.Scripts.Base.JsonParsing
{
    ...
    public static class ObjectConstructor
    {
        ...
        private static object ParseObject(object parsedObject, Type type)
        {
            ...
            Assert.EqualOrLesser(markedConstructors.Count(), 1, "More than a single marked constructor on type {0}".FormatWith(type.Name));

            if (markedConstructors.None())
            {
                Assert.AreEqual(type.GetConstructors().Count(), 1, "More than a single construcor on type {0}".FormatWith(type.Name));
                ...
                if (!objectAsDictionary.TryGetValue(param.Name, out obj))
                {
                    ...
                    else
                    {
                        throw new ArgumentException(
                            "Parameter {0} is missing in {1}".FormatWith(
                                param.Name,
                                objectAsDictionary.ToJoinedString<KeyValuePair<string, object>>(",")));
                    }
                }
